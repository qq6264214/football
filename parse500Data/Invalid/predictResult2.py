...
from changeUtils import point3Arr,point4Arr
#列名,值
colsMap = {}
#sql
conditionsMap = {}
...
def getMaxMins(database,pankou,linchangpankou,bisaileixing,bisaishijian,zhudui,kedui):
    ...
    if not colsMap.__contains__(pankouKey):
        colArr = DB.queryColNameAndValByPankou(database, pankou, linchangpankou,3)
    ...
    for j in range(len(pValList)):
        ...
        for v in tmap[colName]:
            ...
            # if abs(val)>abs(val2) and abs(val)>abs(val3):
