...
def wrapSeniorRules(database, consql):
    ...
    for j in pankouList:
        ...
        totalSql = totalSql + ' AND pankou=%s AND  linchangpankou=%s' % (j[0], j[2])
        count = j[1]
        appendCons(totalSql, count, j[0], j[2], conds, consql, database)

        # DB.insertCondition(database, conds)


def appendCons(totalSql, count, pankou, linchangpankou, conds, constr, database):
    rArr = [[' AND zhubifen>kebifen ', '胜'], [' AND zhubifen=kebifen ', '平'], [' AND zhubifen<kebifen ', '负'],
            [' AND (zhubifen-kebifen-linchangpankou)*linchangpankou>0 ', '上盘'],
            [' AND (zhubifen-kebifen-linchangpankou)*linchangpankou<0 ', '下盘']]
